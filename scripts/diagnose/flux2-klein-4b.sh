#!/usr/bin/env bash
# Launch + smoke-test flux2-klein-4b — 4B transformer, 4 steps, Apache-2.0.
# On 12GB: no offload, no quantization needed (smaller than FLUX.1's 12B).
#
# Run from project root:
#   bash scripts/diagnose_flux2_klein_4b.sh
...
update_env COMPOSE_PROFILES flux2-klein-4b
update_env FLUX2_KLEIN_4B_CPU_OFFLOAD false
update_env FLUX2_KLEIN_4B_SEQUENTIAL_OFFLOAD false
update_env FLUX2_KLEIN_4B_WARMUP false
ok "Env flags set (no offload, no quant, no warmup)"

log "Starting $SERVICE (builds if Dockerfile args changed) …"
"${COMPOSE[@]}" up -d --build "$SERVICE" gateway
ok "Service up issued"

log "Waiting up to ${READY_TIMEOUT}s for gateway to mark flux2-klein-4b available …"
start=$SECONDS
while true; do
    ...
    if grep -q 'Worker ready: flux2-klein-4b.*model is now available' <<<"$gateway_logs"; then
        ok "Gateway marks flux2-klein-4b as available"
        break
    fi
...
http_code=$(curl -s -o "$RESP_JSON" -w '%{http_code}' \
    ...
    -d '{"model":"flux2-klein-4b","prompt":"a cyberpunk cat"}' || echo 000)
elapsed=$(( SECONDS - t0 ))
...
case "$http_code" in
    200)
        ...
        echo "  → Open $OUTPUT to compare against sd35_diag.png."
        echo "  → Subsequent requests should be ~3-8s (4-step distilled pipeline)."
        ;;
    500|504)
        ...
        "${COMPOSE[@]}" logs --no-color --tail 40 "$SERVICE"
        echo
        echo "  → If OutOfMemory: enable nf4 quantization with:"
        echo "      FLUX2_KLEIN_4B_QUANTIZATION=nf4"
        echo "    and add bitsandbytes + build-essential to the worker"
        echo "    (see flux1-schnell setup for the pattern)."
        ;;
    *)
